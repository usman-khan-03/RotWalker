...
// Helper function
const toCoords = (point: { lat: number; lng: number }) => ({
	latitude: point.lat,
	longitude: point.lng,
});
...
export function HeroMap({
	...
	const originCoords = toCoords(origin);
	const destinationCoords = toCoords(destination);

	// Calculate current position along the line
	const currentLat =
		origin.lat + (destination.lat - origin.lat) * progressRatio;
	const currentLng =
		origin.lng + (destination.lng - origin.lng) * progressRatio;
			...
			>
				{lines.map((line, index) => (
					<React.Fragment key={index}>
						<Polyline
							coordinates={[toCoords(line.origin), toCoords(line.destination)]}
						...
						<Marker
							coordinate={{
								latitude:
									line.origin.lat +
									(line.destination.lat - line.origin.lat) * line.progressRatio,
								longitude:
									line.origin.lng +
									(line.destination.lng - line.origin.lng) * line.progressRatio,
							}}
				...
				{/* Origin marker */}
				<Marker coordinate={originCoords} pinColor={theme.colors.textMuted} />

				{/* Destination marker */}
				<Marker coordinate={destinationCoords} pinColor={getColor(mode)} />
			</MapView>
		</View>
	);
}
